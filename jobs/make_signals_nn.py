...
"""
nn_predict(symbols_csv: Optional[str]) -> dict
Returns per-symbol predictions based on the ACTIVE GradientBoosting model in models_meta.
If symbols_csv is None, it will use the SYMBOLS env var.
"""
...
P_BUY = float(os.getenv("ML_BUY_THRESHOLD", "0.53"))
P_SELL = float(os.getenv("ML_SELL_THRESHOLD", "0.47"))
MAX_HOLD_BIAS = float(os.getenv("ML_HOLD_BAND", "0.02"))

# ------------------------------ helpers ------------------------------

def _active_artifact_path() -> Optional[str]:
...
def nn_predict(symbols_csv: Optional[str] = None) -> dict[str, dict]:
    """Return per-symbol predictions: side + strength"""
    symbols = (
        symbols_csv.split(",")
        if symbols_csv
        else os.getenv("SYMBOLS", "AAPL,MSFT,SPY").split(",")
    )
