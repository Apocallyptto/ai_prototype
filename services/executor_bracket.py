...
TICK_SIZE = float(os.getenv("TICK_SIZE", "0.01"))  # for SL/TP validity
...
def _flatten_yf_multiindex(df: pd.DataFrame, symbol: str) -> pd.DataFrame:
    ...
    try:
        ...
        if sym_lower in lv0:
            return df.xs(key=sym_lower, level=0, axis=1)
    except Exception:
        pass
    # fallback: drop one level
    try:
...
def _qty_from_risk(price: float, atr: float) -> float:
    ...
    if FRACTIONAL:
        ...
        if max_notional <= 0:
            return 0.0
        return max_notional / max(price, 0.01)
    else:
    ...
    for _, row in df.iterrows():
        ...
        if side == "sell":
            if LONG_ONLY:
                ...
                continue
            if not short_ok:
                log.info("%s: account shorting not enabled -> skip short", sym)
                continue
        ...
        # --- bracket targets anchored to entry (limit_px) + min tick enforcement ---
        entry = float(limit_px)
        tick = max(TICK_SIZE, 0.01)
        if side == "buy":
            tp_px = entry + TP_ATR_MULT * atr
            sl_px = entry - SL_ATR_MULT * atr
            # Ensure TP above entry, SL below entry by at least one tick
            tp_px = max(tp_px, entry + tick)
            sl_px = min(sl_px, entry - tick)
        else:
            tp_px = entry - TP_ATR_MULT * atr
            sl_px = entry + SL_ATR_MULT * atr
            # Ensure TP below entry, SL above entry by at least one tick
            tp_px = min(tp_px, entry - tick)
            sl_px = max(sl_px, entry + tick)
        ...
        try:
            ...
            log.info("submitted %s %s %s px=%.2f tp=%.2f sl=%.2f id=%s",
                     sym, side, sized, float(entry), float(tp_px), float(sl_px), getattr(o, "id", "n/a"))
        except Exception as e:
