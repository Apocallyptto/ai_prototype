...
def fetch_new_signals() -> List[dict]:
    ...
    # očistíme symboly a spravíme z nich "IN ('AAPL','MSFT','SPY')"
    symbols_clean = [s.strip() for s in SYMBOLS if s.strip()]
    symbols_sql = ",".join(f"'{s}'" for s in symbols_clean)
    ...
    with engine.begin() as conn:
        rows = conn.execute(text(sql_str)).mappings().all()
...
def create_limit_order(symbol: str, side: str, strength: float):
    ...
    # Price adjustment podľa ATR_PCT
    if side.lower() == "buy":
