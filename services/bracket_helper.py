...
# Optional: size curve by strength (new)
...
SIZE_EXP               = float(os.getenv("SIZE_EXP", "2.0"))         # >1 conservative, <1 aggressive
SIZE_MIN_MULT          = float(os.getenv("SIZE_MIN_MULT", "0.50"))   # 50% of base at low end
SIZE_MAX_MULT          = float(os.getenv("SIZE_MAX_MULT", "1.00"))   # 100% at/above high end
...
def _atr(symbol: str, period: int, lookback_days: int) -> float:
    ...
    # single-row Series -> floats (iloc[0]) to avoid FutureWarning
...
def _atr_exits(symbol: str, side: str, base_price: float) -> Tuple[float, float]:
    ...
    # choose the more conservative levels
    if side == "buy":
        tp = min(tp, tp_cap)
        sl = max(sl, sl_cap)
    else:
...
def _risk_per_share(symbol: str, side: str, base_price: float) -> float:
    ...
    if side == "buy":
        rps = max(0.01, base_price - sl)
    else:
        rps = max(0.01, sl - base_price)
    return float(rps)
...
def _last_quote(symbol: str) -> float:
    ...
    r.raise_for_status()
    js = r.json()
    # Use bid/ask mid if last not present
    q = js.get("quote", {})
    ...
    import yfinance as yf
    t = yf.Ticker(symbol)
    px = t.fast_info.last_price or t.info.get("regularMarketPrice")
    return float(px)
...
def submit_bracket(
    ...
    *,
    ...
    # dynamic size (optionally scaled by strength)
    if qty is None:
    ...
    # order type: if during RTH and we have a reasonable last, place LIMIT near base
    order_type = "limit"
    payload = {
        "symbol": symbol,
        "side": side,
    ...
    log.info("submit bracket (atr(period=%s,tp×%s,sl×%s), dyn=%s) -> %s",
             ATR_PERIOD, ATR_MULT_TP, ATR_MULT_SL, "True" if USE_DYNAMIC_SIZE else "False", payload)
