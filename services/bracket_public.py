...
It tries to bind to your existing implementation under multiple likely names.
If a public function is missing in bracket_helper, we provide a safe fallback
(using yfinance for prices/ATR) so tools can still run.
...
- submit_bracket(...) -> delegate to your helper if available; otherwise NotImplementedError
"""
...
def _yf_last_price(symbol: str) -> float:
    try:
        ...
        raise RuntimeError(
            "get_last_price fallback needs yfinance. Please install `yfinance` "
            "or expose a get_last_price in services.bracket_helper."
        ) from e
    t = yf.Ticker(symbol)
    # fast path: regularMarketPrice is usually present
    info = t.fast_info if hasattr(t, "fast_info") else {}
    px = getattr(info, "last_price", None) or getattr(info, "last", None) or info.get("last_price") if isinstance(info, dict) else None
    if px is not None:
        return float(px)

    # slow path: recent 1d/1m data
...
def _yf_atr(symbol: str, period: int, lookback_days: int) -> float:
    try:
        import yfinance as yf
        import pandas as pd  # noqa: F401 (only to ensure pandas exists)
    except Exception as e:
        raise RuntimeError(
            "atr_value fallback needs yfinance (and pandas). Install them or "
            "expose atr_value/_atr in services.bracket_helper."
        ) from e
...
def _fallback_risk_per_share(symbol: str, side: str, last_price: float) -> float:
    # Classic R per-share = ATR_MULT_SL * ATR
...
def atr_value(symbol: str, period: int, lookback_days: int) -> float:
    if _atr_fn:
        # be tolerant: some helpers only take (symbol, period) or (symbol,)
        try:
...
def submit_bracket(*args, **kwargs):
    if _submit_bracket_fn:
        return _submit_bracket_fn(*args, **kwargs)
