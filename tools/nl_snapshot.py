...
def upsert_daily_pnl(dsn: str, as_of: date, equity: float) -> None:
    with psycopg2.connect(dsn=dsn) as conn, conn.cursor() as cur:
        ...
        row = cur.fetchone()
        if row is None:
            # First snapshot of the day → profit = 0
            profit = 0.0
            profit_pct = 0.0
            cur.execute("""
            ...
            # Update the single row of the day to latest equity and profit
            cur.execute("""
...
def main():
    ...
    try:
        ...
        print(f"ERROR: failed to fetch equity from Alpaca: {e}", file=sys.stderr)
        sys.exit(2)
